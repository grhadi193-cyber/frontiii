...
function AddressForm({ token, onSaved, onCancel }) {
  ...
  const [form, setForm] = useState({
    province: '',     // نام استان — این به بک‌اند می‌رود
    province_id: '',  // فقط برای load کردن شهرها
    city: '',         // نام شهر — این به بک‌اند می‌رود
    city_id: '',      // فقط داخلی
    street: '',
    postal_code: '',
    title: 'خانه',
...
if (!form.city)     return setError('شهر را انتخاب کنید')
    ...
    setLoading(true)
    try {
      // ✅ فقط فیلدهایی که AddressIn بک‌اند می‌شناسد
 const payload = {
...
function StepShipping({ token, address, items, onNext, onBack }) {
  ...
  useEffect(() => {
    ...
    // ✅ بک‌اند province/city (نام) می‌گیرد، نه id
    calculateShipping({
      ...
      .then(data => {
        const list = Array.isArray(data) ? data : (data.methods || data.results || [])
        setMethods(list)
        if (list.length > 0) setSelected(list[0])
      })
...
function StepConfirm({ token, items, address, shipping, totalPrice, onBack, onPaid }) {
  ...
  return (
    <div>
      ...
      {/* Address — ✅ بک‌اند province/city برمی‌گرداند (نه province_name/city_name) */}
      <div className="bg-gray-50 rounded-2xl p-4 mb-4">
    ...
    <div className="min-h-screen bg-[#F8FAFC] py-8 px-4">
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
          {step === 1 && (
            <StepAddress token={token} onNext={addr => { setAddress(addr); setStep(2) }} />
          )}
          {step === 2 && (
            <StepShipping
              token={token}
              address={address}
              items={items}
              onNext={method => { setShipping(method); setStep(3) }}
              onBack={() => setStep(1)}
            />
          )}
          {step === 3 && (
